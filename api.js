...
server.get("/v1/scenes", (req, res) => {
  if (req.query.author) {
    pool.query(
      "SELECT * FROM `scenes` WHERE author = ?",
      req.query.author,
      function (error, results) {
        if (error) {
          logError(error);
    ...
    pool.query("SELECT * FROM `scenes`", function (error, results) {
      if (error) {
        logError(error);
        res.sendStatus(500);
      } else if (results.length === 0) {
...
server.put("/v1/scene", (req, res) => {
  ...
  pool.query(
    "SELECT * FROM `users` WHERE authorKey = ?",
    req.body.authorKey,
    (error, users) => {
      if (error) {
            ...
            // old scene found, lets update it
              ...
              if (scenes[0].creatorId === users[0].userId) {
                ...
                res
                  .status(403)
                  .send(
                    `Use another id. Id ${scene.id} is already used in ${scene.title} by ${scene.author}.`
                  );
              }
            }
          }
  ...
  pool.query(
    ...
    (error, users) => {
      ...
      // user found
        ...
        pool.query(
          "DELETE FROM `scenes` WHERE id = ? AND creatorId = ?",
...
function logError(error, requestId = "no-id") {
  console.error(
    `ERR ${requestId} ${new Date().toISOString()} ${
