...
const behaviours = {
    ...
    idles: {
        ...
        do({ sprite }) {
            if (sprite.idleChangeCounter === 0 && sprite.collisionData.bottom && sprite.velocity.x > -0.03 && sprite.velocity.x < 0.03) {
                sprite.changeAnimation({ animation: "idle", randomFrame: true });
                sprite.idleChangeCounter = 60;
            }
    ...
    fails: {
        do({ nextState }) {
            ...
            if (!state.failed) {
                ...
                sprites.filter(sprite => sprite.isPlayer).forEach(sprite => {
                    //sprite.energy = 0;
    ...
    jumps: {
        init(sprite) {
            ...
            sprite.jumpPauseCounter = Math.abs(sprite.x) % Math.max(1, sprite.jumpPause);
            sprite.groundContactCounter = 0;
            sprite.jumpAgain = true;
        },
        do({ sprite, state }) {

            if ((!sprite.isPlayer && !sprite.isEnemy) || sprite.freezeCounter > 0 || sprite.energy < 1 || state.completed) {
                return;
            }
            ...
            const jump = (sprite.isPlayer && controls.up) || (sprite.isEnemy && sprite.jumpPauseCounter === 0);

            if ((sprite.jumpAgain && sprite.groundContactCounter > 0) || sprite.jumpCounter > 0) {
    ...
    moves: {
        ...
        do({ sprite }) {
            ...
            if (!sprite.isPlayer || sprite.freezeCounter > 0 || sprite.energy < 1 || state.completed) {
                return;
            }
            ...
            if (controls.right) {
                ...
                if (sprite.collisionData.bottom) {
                    ...
                    if ((sprite.velocity.x > 2 && Math.random() < 0.1) || sprite.velocity.x < 0) {
                        postMessage({
                            type: "EMIT_PARTICLES",
                            particleTypeId: "dust",
                            amount: 2,
    ...
    "changes-collectors-size": {
        ...
        do({ sprite, sprites }) {
            ...
            sprite.collisionData.spriteIds.forEach(id => {
                ...
                if (target && !target.isEnemy && target.isCollector && target.transform && target.energy > 0) {
                    ...
                    if (sprite.transform.width > 0 || sprite.transform.height > 0) {
                        ...
                        postMessage({ type: "PLAY_SOUND", sound: "transform-down", x: target.x, y: target.y });
                    }
    ...
    "changes-players-props": {
        do({ sprite, sprites }) {
            sprite.collisionData.spriteIds.forEach(id => {
                const target = sprites.find(sprite => sprite.id === id);
                if (target && target.isPlayer && target.energy > 0) {
                    for (prop in sprite.transform) {
                        target[prop] = sprite.transform.spriteProps[prop];
                    }
                    if (sprite.transform.sound) {
                        postMessage({ type: "PLAY_SOUND", sound: sprite.transform.sound, x: target.x, y: target.y });
                    }
                    target.freezeCounter = 30;
                }
            });
    ...
    "shoots-fireball": {
        ...
        do({ sprite }) {
            ...
            if (sprite.reloadCounter > 0) {
                ...
                addSprite({
                    typeId: "fireball",
                    x: sprite.direction === DIRECTION.LEFT ? sprite.x : sprite.x + sprite.width - sprite.fireBallSize / 2,
                ...
                postMessage({ type: "PLAY_SOUND", sound: "fire", x: sprite.x, y: sprite.y });
            ...
            if (sprite.energy > 0) {
                ...
                sprite.collisionData.spriteIds.forEach(id => {
                    ...
                    if (target && target.isEnemy !== sprite.isEnemy) {
                        ...
                        target.energy--;
                        target.damageCounter = 30;
                        postMessage({ type: "PLAY_SOUND", sound: "damage", x: target.x, y: target.y });
                    }
                });
    ...
    "moves-forward": {
        ...
        do({ sprite, state }) {
            ...
            if ((sprite.collisionData.bottom || sprite.weight === 0) && sprite.velocity.x !== 0) {
    ...
    "moves-stops": {
        ...
        do({ sprite, state }) {
            ...
            /*
            else if (sprite.stopCounter > 0) {
                sprite.stopCounter--;
            }
            */
            else /*if (sprite.collisionData.playerHits > 0)*/ {
    ...
    "floats": {
        init(sprite) {
            ...
            if (sprite.originalFloatY === undefined) {
                ...
                sprite.y = sprite.originalFloatY + Math.sin(sprite.floatCounter / sprite.floatStep) * sprite.floatFactor;
            }
        },
        do({ sprite }) {
    ...
    "changes-type-when-completed": {
        do({ sprite, state }) {
            if (sprite.energy > 0 && state.completed) {
                ...
                postMessage({ type: "PLAY_SOUND", sound: "transform-up", x: sprite.x, y: sprite.y });
            }
        }
    },
    ...
    "moves-forward-if-touched": {
        ...
        do({ sprite }) {
            if (sprite.collisionData.bottom
                && (sprite.collisionData.playerHits > 0 || sprite.collisionData.enemyHits > 0)) {

                if (sprite.velocity.x === 0) {
                    postMessage({ type: "PLAY_SOUND", sound: "alert", x: sprite.x, y: sprite.y });
                }
    ...
    "changes-pattern-when-energy-low": {
        ...
        do({ sprite }) {
            if (sprite.energy <= sprite.transform.energyThreshold && !sprite.energyLowPatternChanged && sprite.pattern !== sprite.transform.pattern) {
    ...
    "removed-if-energy-0": {
        ...
        do({ sprite }) {
            if (sprite.energy <= 0 && !sprite.remove) {
                sprite.remove = true;
                postMessage({ type: "PLAY_SOUND", sound: "collect", x: sprite.x, y: sprite.y });
                postMessage({
                    type: "EMIT_PARTICLES",
            ...
            if (!sprite.accessory.targetId) {
                sprite.collisionData.spriteIds.forEach(id => {
                    ...
                    if (target && target.isPlayer && target.energy > 0) {
                        ...
                        postMessage({ type: "PLAY_SOUND", sound: "pop", x: sprite.x, y: sprite.y });
                        sprite.energy = 0;
                    }
                });
            }
                ...
                if (target) {
                    sprite.x = target.x + target.width * sprite.accessory.x - sprite.width / 2;
                    sprite.y = target.y + target.height * sprite.accessory.y - sprite.height / 2;
                }
            }
        }
    },
    ...
    "kills-falling": {
        do({ sprite, sprites }) {
            sprite.collisionData.spriteIds.forEach(id => {
                ...
                if (target && target.isPlayer && target.velocity.y > 0.1 && target.y < sprite.y && target.energy > 0) {
                    //target.remove = true;
                    target.damageCounter = 30;
                    target.energy = 0;
                    postMessage({
    ...
    "collects": {
        ...
        do({ sprite, sprites }) {
            ...
            sprite.collisionData.spriteIds.forEach(id => {
                const target = sprites.find(sprite => sprite.id === id);
                if (target && target.energy > 0 && spriteTypes[target.typeId].collectable) {
                    target.energy = 0;
    ...
    "follows-player": {
        do({ sprite, sprites }) {
            ...
            const players = sprites.filter(sprite => sprite.isPlayer && sprite.energy > 0);

            players.forEach(player => {
                if (Math.abs(player.x + player.width / 2 - sprite.x + sprite.width / 2) <= 500
                    && player.y + player.height > sprite.y && player.y < sprite.y + sprite.height) {
    ...
    "dies-if-falls": {
        do({ sprite }) {
            if (sprite.energy > 0 && sprite.collisionData.bottom && Math.abs(sprite.velocity.y) > 0.5) {
                sprite.energy = 0;
            }
        }
    },
