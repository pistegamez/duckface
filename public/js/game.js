...
const initGame = async ({
  ...
  document
    ...
    .addEventListener("touchstart", onGameTouchStartListener);
  document
    .getElementById("game-canvas")
    .addEventListener("touchend", onGameTouchEndListener);
...
function saveSceneBestTime(sceneId, version = "1.0", sceneCompletionTime) {
  ...
  const majorVersionNumber = version.split(".")[0];
  bestTimes[
    `${album.id}-${sceneId}-${majorVersionNumber}`
  ] = sceneCompletionTime;
...
function moveCamera() {
  ...
  if (targets.length > 0) {
    ...
    targetX = Math.min(
      scene.right - canvas.width,
      Math.max(scene.left, targetX)
    );
    targetY = Math.min(
      scene.bottom - canvas.height,
      Math.max(scene.top, targetY)
    );
  ...
  camera.x = Math.min(
    scene.right - canvas.width,
    Math.max(scene.left, camera.x)
  );
  camera.y = Math.min(
    scene.bottom - canvas.height,
    Math.max(scene.top, camera.y)
  );
...
function emitParticles({ particleTypeId, particleProps, amount }) {
  for (let i = 0; i < amount; i++) {
    if (
      particleProps.x > camera.pixelX &&
      particleProps.x < camera.pixelX + canvas.width &&
      particleProps.y > camera.pixelY &&
      particleProps.y < camera.pixelY + canvas.height
    ) {
...
function moveParticles() {
  ...
  particles = particles.filter(
    (particle) => particle.startTime + particle.duration > Date.now()
  );
}
