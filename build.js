...
fs.readdirSync("./public/albums").forEach(albumDir => {
  if (albumDir !== "albums.json") {
    copyFile(`./public/albums/${albumDir}/album.json`, `./build/albums/${albumDir}/album.json`);
...
function createGameBundle() {
  createBundle(
    [
      "./public/js/common.js",
      ...fs.readdirSync("./public/sprite-types").map(file => './public/sprite-types/' + file),
...
fs.readFile('./public/game.html', 'utf8', function (err, data) {
  ...
  let result = data.slice(0, scriptsStart) + `<script src="js/${GAME_BUNDLE_FILE}"></script>` + data.slice(scriptsEnd + 21);
...
fs.readFile('./public/editor.html', 'utf8', function (err, data) {
  ...
  let scriptsStart = data.indexOf('<!-- scripts-start -->');
  let scriptsEnd = data.indexOf('<!-- scripts-end -->');
  let result = data.slice(0, scriptsStart) + `<script src="js/${EDITOR_BUNDLE_FILE}"></script>` + data.slice(scriptsEnd + 21);
  result = result.replace('worker.js', WORKER_FILE);
...
fs.readFile('./public/js/worker.js', 'utf8', function (err, data) {
  ...
  let scriptsStart = data.indexOf('// scripts-start');
  let scriptsEnd = data.indexOf('// scripts-end');
  let result = data.slice(0, scriptsStart) + `importScripts('/js/${WORKER_BUNDLE_FILE}');` + data.slice(scriptsEnd + 14);
...
function createBuildInfo() {
  ...
  fs.writeFile(`./build/build-info.json`, JSON.stringify(info), 'utf8', function (err) {
    if (err) return console.log(err);
